...
OF calc_bias()
{
    ...
    for (yp = YS-halfwidth; yp < YS+halfwidth; yp+=4) {
        OF   *p1;
        OF   *cnt;
 
	p1 = &sums[yp][XS - halfwidth];
        cnt = &counts[yp][XS - halfwidth];
        for (x = 0; x < halfwidth*2; x+=8) {
            sum = 0; 
	    for (k = 0; k < 16; k++) { 
	    	v1 = *p1;
	    	v1 /= *cnt;
...
double fsharpness()
{
	...
	cnt1 = &counts[yp][XS - halfwidth];
	cnt0 = &counts[yp-1][XS-halfwidth];
	cnt2 = &counts[yp+1][XS-halfwidth];
...
void add_imagex(int idx, int dx, int dy)
{
	...
	sum_ptr = &sums[y + dy + YS/2][XS/2 + dx];
        cnt_ptr = &counts[y + dy + YS/2][XS/2 + dx];
 
        vector_add (&input[y*XS] + dh, sum_ptr + dh, cnt_ptr + dh, hs);
    }
}
...
void sub_imagex(int idx, int dx, int dy)
{
    ...
    //for (y = 0; y < YS; y++) {
        ...
        vector_sub (&input[y*XS] + dh, sum_ptr + dh, cnt_ptr + dh, hs);
    }
}
...
void load_image(char *fn, long idx)
{
    ...
    fseek(input,XS*YS*2L*(idx)/4 , SEEK_SET);
    ...
    for (y = 0; y < YS/2; y++) {
        int v = fread(buffer, 2, XS/2, input);
	    ...
	    swap1 += 100.0;
	    cur_buffer[(2 * y) * w + (2 * x)] = swap1;
            cur_buffer[(2 * y) * w + (2 * x) + 1] = swap1; 
	    cur_buffer[((2 * y) + 1) * w + (2 * x)] = swap1;
            cur_buffer[((2 * y) + 1) * w + (2 * x) + 1] = swap1;
...
int main(int argc, char **argv)
{
    ...
    int v = fread(dark, sizeof(float), XS*YS/4, file);
    fclose(file);
	...
	selected[i-1] = 1;	
	if (selected[i - 1])
	  add_image(i - 1, round(2.0 * dxs[i - 1]), round(2.0 * dys[i - 1]));
    }
