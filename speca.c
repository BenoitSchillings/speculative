...
#define XS 512
#define YS 512
#define MAX_INPUT 49900
#define MAX_POOL  5000
//---------------------------------------------------------------------
...
void set_fbias()
{
  ...
  for (x = (xs2-100); x < (xs2+100); x += 30) {
    for (y = (ys2-100); y < (ys2+100); y += 30) {
      if (sums[x][y] < fbias) {
	fbias = sums[x][y];
      }
    }
...
char loaded(int victim)
{
  return (inputs[victim] != 0);
}
...
void load_image(char *fn, long idx)
{
  ...
  fseek(file,(512L*512L*2L*(idx)) , SEEK_SET);
